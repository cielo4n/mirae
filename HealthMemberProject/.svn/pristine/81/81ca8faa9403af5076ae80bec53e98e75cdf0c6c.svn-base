...
public class HSMemberUpdateDialog extends JDialog implements ActionListener {
	...
	public HSMemberUpdateDialog(HealthScreen owner) {
		...
		// StaticValues에 저장되어 있던 값 읽어서 텍스트 필드에 뿌림
...
//				.trainerCombo(StaticValues.getSvGym_id()));
		...
		//StaticValue에 저장된 트레이너 번호와 이름을 가져와서, 콤보박스에서 트레이너를 선택
		cb_health_memberupdate_trainer.setSelectedItem(StaticValues.getSvMemDto().getTrainer_no()
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (obj == btn_health_member_ok) { // 확인을 누르면
			...
			//날짜가 잘 입력되었나 확인
			chkday = WarningMessage.checkDay(mem_year, mem_month, mem_day);
			if(chkday == 1){
